...
class facenet(nn.Module):
    ...
    def forward(self, x):
        x =                   self.lrn(self.maxpool(F.relu(self.conv1(x))))
        x = self.maxpool(self.lrn(F.relu(self.conv2(F.relu(self.conv2a(x))))))
        x =          self.maxpool(F.relu(self.conv3(F.relu(self.conv3a(x)))))
        x =                       F.relu(self.conv4(F.relu(self.conv4a(x))))
        x =                       F.relu(self.conv5(F.relu(self.conv5a(x))))
        x =          self.maxpool(F.relu(self.conv6(F.relu(self.conv6a(x)))))
        x = x.view(x.size(0), -1)
        x = self.embedding(x)
