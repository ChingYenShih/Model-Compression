...
class MyDataset(Dataset):
    def __init__(self, img, id, mapping, dataAUG, triplet):
        self.transform = trans.Compose([
                        trans.RandomHorizontalFlip(),
                        trans.CenterCrop(120),
                        trans.RandomRotation(30),
        ...
        self.val_transform = trans.Compose([
                        trans.CenterCrop(120),
                        trans.ToTensor(),
                        trans.Normalize(mean=[0.5, 0.5, 0.5],
                                         std=[0.5, 0.5, 0.5])
    ...
    def __getitem__(self, index):
        if self.triplet:
            ...
            if self.dataAUG:
                ...
                img = self.val_transform(self.x[index])
            id = self.y[index]
            return img,id
    def __len__(self):
        return len(self.x)

def get_data_loader(x, y, mapp, batch_size = 32, shuffle = True, dataAUG = False, triplet=False):
    dataset = MyDataset(x, y, mapp, dataAUG, triplet)
    loader = Data.DataLoader(
        dataset = dataset,
        batch_size = batch_size,
        shuffle = shuffle
